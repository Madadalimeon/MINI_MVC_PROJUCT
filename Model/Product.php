...
class Product {
    ...
    public function addProduct($name, $price, $stock, $image, $Products_add_time) {
        $query = "INSERT INTO $this->table (Products_name, Products_img, Products_price, Products_Stock, Products_add_time) 
                  VALUES (?, ?, ?, ?, ?)";
    ...
    public function updateProduct($id, $name, $price, $stock, $image, $Products_add_time) {
        $query = "UPDATE $this->table 
                  SET Products_name=?, Products_img=?, Products_price=?, Products_Stock=?, Products_add_time=? 
                  WHERE Products_id=?";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("ssdisi", $name, $image, $price, $stock, $Products_add_time, $id);
        return $stmt->execute();
    }
